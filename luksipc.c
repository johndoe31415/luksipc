...
static uint64_t getDiskSize(int aFd) {
	uint64_t result;
	if (ioctl(aFd, BLKGETSIZE64, &result) == (uint64_t)-1) {
...
static void showProgress(const struct copyProgress *aProgress, bool aShow) {
	...
	if (startTime < 0) {
		...
		if (aShow) {
			double timeDiff = curTime - startTime;
			int timeDiffI = (int)timeDiff;
			logmsg(LLVL_INFO, "%2d:%02d: "
							...
							"%7lu MB left"
							"\n", 
								(int)(timeDiffI / 3600), (timeDiffI % 3600 / 60),
								100.0 * (double)aProgress->outOffset / (double)aProgress->totalSize, 
								aProgress->outOffset / 1024 / 1024, 
								aProgress->totalSize / 1024 / 1024, 
								(double)aProgress->copied / timeDiff / 1024. / 1024.,
								(aProgress->totalSize - aProgress->outOffset) / 1024 / 1024
			);
			lastPos = aProgress->outOffset;
		}
	}
...
static void luksClose(struct copyProgress *aProgress) {
	int result;
...
static bool copyDisk(struct copyProgress *aProgress) {
	...
	while (true) {
		...
		if (bytesToRead > 0) {
			...
			logmsg(LLVL_DEBUG, "%d bytes to read at inOffset = %ld remaining = %ld\n", bytesToRead, aProgress->inOffset, aProgress->remaining);
		}
...
static void convert(struct parameters const *parameters) {
	...
	if (ioctl(progress.cryptDevFd, BLKGETSIZE64, &cryptDevSize) == (uint64_t)-1) {
		perror("ioctl BLKGETSIZE64");
		chunkWriteAt(&progress.dataBuffer[0], progress.rawDevFd, 0);		/* Try to unpulp disk */
		exit(EXIT_FAILURE);
	}
	 ...
	 * already) */
	logmsg(LLVL_INFO, "Size of cryptodisk is %lu bytes (%lu MB + %lu bytes)\n", cryptDevSize, cryptDevSize / (1024 * 1024), cryptDevSize % (1024 * 1024));
	{
...
static void checkPreconditions(struct parameters const *parameters) {
	...
	if (!parameters->batchMode) {
		...
		if (!parameters->resume) {
			...
			fprintf(stderr, "   2. You have ensured secure storage of the keyfile\n");
			fprintf(stderr, "   3. Power conditions are satisfied (i.e. your Laptop is not running off battery)\n");
			fprintf(stderr, "   4. You have a backup of all data on that device\n");
			fprintf(stderr, "\n");
			fprintf(stderr, "    %s: %lu MB = %.1f GB\n", parameters->device, devSize / 1024 / 1024, (double)(devSize / 1024 / 1024) / 1024);
